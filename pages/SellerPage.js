...
// import { QRCodeCanvas } from 'qrcode.react'; // Not directly used for display here
...
const SellerPage = () => {
  ...
  const [usdtAmountUSD, setUsdtAmountUSD] = useState('');
  const [qrCodeValueInternal, setQrCodeValueInternal] = useState(''); 
  ...
  const handleUsdtAmountChange = (value) => {
    if (!/^\\d*\\.?\\d*$/.test(value) && value !== '') return; // Corrected Regex
  ...
  const generateQrCode = async () => {
    ...
    if (isNaN(numTotalPrice) || numTotalPrice <= 0) {
      ...
      genErrorMessage = 'USDT amount cannot exceed 50% of total price.';
        ...
        if (numIvyAmountUSD < (numTotalPrice * 0.5) - 0.001) { // Check with a small tolerance
    ...
    const qrDataForEncoding = {
      productImageBase64: imageBase64,
      productName: '', // Kept for structure, not displayed/input
    ...
    if (finalQrStringToEncode.length > MAX_QR_STRING_LENGTH) {
      if (imageBase64) {
        const qrDataWithoutImage = { ...qrDataForEncoding, productImageBase64: null };
  ...
  const resetForm = () => {
    ...
    if (fileInput) fileInput.value = ''; // No 'as HTMLInputElement' needed
    const cameraInput = document.getElementById('cameraUploadInput');
    if (cameraInput) cameraInput.value = ''; // No 'as HTMLInputElement' needed
  };
